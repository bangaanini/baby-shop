...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'ringkasan' | 'produk' | 'pesanan' | 'promo'>('ringkasan');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50 text-slate-800">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Admin Header */}
        <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 mb-8 shadow-md">
                    ...
                    {ord.status === 'diproses' && (
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="px-2.5 py-1 text-xs bg-white border border-slate-200 rounded-lg w-36"
                        />
                        <button
                          onClick={() => handleShipOrder(ord.id)}
